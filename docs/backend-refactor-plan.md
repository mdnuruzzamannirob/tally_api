...
- `pnpm test:smoke` — passed against the rebuilt runtime image with the
  documented local development environment and disposable database;
- `git diff --check` — passed.

During this gate, integration execution was made deterministic by running
database files with `--no-file-parallelism`; the integration suite clears
shared tables between files, so parallel file execution caused test-only
foreign-key races. The dashboard follow-up envelope, user profile/preferences
response envelopes, CSV escaping expectation, OAuth verified-user fixtures,
and request-ID-aware response assertions were aligned with the current
contracts. A Node 20 runtime smoke also exposed and fixed the CommonJS-safe
