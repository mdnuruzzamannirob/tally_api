...
export class GoogleOAuthService {
  ...
  async complete(
    ...
    const oauthState = await this.repository.consumeState("GOOGLE", hashToken(state), new Date());
    ...
    if (oauthState.intent === "LINK") {
      ...
      if (result === "account-owned-by-another-user")
        throw new ApiError(409, "CONFLICT", "This Google account is already connected.");
